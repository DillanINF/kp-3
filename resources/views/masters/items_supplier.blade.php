...
@section('page_title', 'Data Barang Supplier')
@section('page_description')Master data barang supplier (barang mentah/bahan baku) untuk proses produksi.@endsection
    ...
    <div class="rounded-xl border border-slate-200 bg-white">
        <div class="flex items-center justify-between border-b border-slate-200 p-4">
            ...
            @if(auth()->user()?->role === 'admin')
                <button type="button" data-open-modal="modal-tambah-item" data-item-type="supplier" data-supplier-id="{{ $selectedSupplierId }}" class="inline-flex items-center gap-2 rounded-md bg-slate-900 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-800">Tambah</button>
            @endif
        </div>
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-slate-200">
                    @forelse($supplierItems as $item)
                        <tr>
                            <td class="px-4 py-3 text-slate-700">{{ $item->supplier?->name ?? '-' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ auth()->user()?->role === 'admin' ? 7 : 6 }}" class="px-4 py-6 text-center text-sm text-slate-500">Belum ada data barang supplier.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div id="modal-tambah-item" data-modal class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/50 p-4">
            <div class="w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-xl">
                <div class="bg-slate-900 px-6 py-5">
                    ...
                    <div class="mt-1 text-sm text-slate-200">Isi master data barang supplier.</div>
                </div>
                ...
                <form action="{{ route('masters.items.store') }}" method="POST" class="space-y-4 px-6 py-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="supplier_id" data-add-item-supplier class="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none" required>
                            @foreach($suppliers as $supplier)
        ...
        <div id="modal-edit-item" data-modal class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/50 p-4">
            <div class="w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-xl">
                <div class="bg-slate-900 px-6 py-5">
                    ...
                    <div class="mt-1 text-sm text-slate-200">Perbarui data barang supplier.</div>
                </div>
                ...
                <form data-edit-item-form data-action-template="{{ route('masters.items.update', ['item' => '__ID__']) }}" action="" method="POST" class="space-y-4 px-6 py-6">
                    ...
                    <div class="space-y-2">
                        <label class="text-sm font-semibold text-slate-700">Supplier</label>
                        <select name="supplier_id" data-edit-item-supplier class="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none" required>
                            @foreach($suppliers as $supplier)
                                <option value="{{ $supplier->id }}">{{ $supplier->name }}</option>
                            @endforeach
                        </select>
